...
两个配套的 agent skill：先把简历整理成结构化的简历文件夹，再用 Agent 应用内的浏览器填招聘网站的表单。

> ⚠️ 使用 LLM 填写简历存在信息泄露风险，请谨慎考虑使用本仓库。
...
- **job-form-filling** —— 读简历文件夹，把招聘网站的在线简历或投递表单填好。按模块填写、逐个保存并回读核对，不自动提交。
  ...
  **这个 skill 只用应用内浏览器**（当前 Agent 应用自带的浏览器）：每个会话各有一份独立的浏览器，所以多个会话可以同时填不同的网站，附件上传也走同一条通道。
...
3. **需要一个自带浏览器的 Agent 应用**：`job-form-filling` 靠应用内的浏览器操作页面，开箱可用，并支持多会话并行（Codex 桌面版的 @Browser 就属于这一类通道）。
...
```text
用 job-form-filling 帮我用应用内的浏览器把这个招聘网站的表单填好，填完先别提交：https://example.com/apply
```

需要同时填多个网站时，每个网站开一个会话，各用各的应用内浏览器并行填，互不干扰。登录、短信验证码和上传附件的确认仍然要你自己来。
